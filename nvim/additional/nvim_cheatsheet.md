...
_Combine these objects with actions like `d` (delete), `c` (change), `y` (yank), or `v` (visual)._
...
- **`q` (Quote):** `ciq` (Change Inside _any_ Quote - auto-detects `'`, `"`, or `` ` ``)
- **`b` (Bracket):** `cib` (Change Inside _any_ Bracket - auto-detects `()`, `[]`, or `{}`)

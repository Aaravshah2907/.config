...
function M.clean_todo_line(line)
  ...
  if found_kw then
    ...
    if vim.tbl_contains({ "TODO", "FIXME", "BUG", "HACK", "NOTE", "PERF", "WARN", "WARNING", "IDEA", "XXX" }, upper_kw) then
      keyword = upper_kw
      text = rest
    end
  end
...
function M.extract_cursor_todo()
  ...
  vim.ui.input({
    ...
    M.add_task_to_tuxedo(input, function(success)
      if success then
        vim.notify(string.format("Task added to Tuxedo (+%s)", project_tag), vim.log.levels.INFO, { title = "Tuxedo" })
      else
...
function M.project_todos(opts)
  ...
  M.fetch_tasks(filter_term, function(tasks)
    if #tasks == 0 then
      ...
      vim.notify("No tasks found for +" .. project_tag .. ", showing all tasks", vim.log.levels.INFO, { title = "Tuxedo" })
      M.all_todos(opts)
      return
    end
...
function M._open_telescope_picker(prompt_title, tasks)
  ...
  local make_display = function(entry)
    ...
    if task.priority == "A" then pri_hl = "DiagnosticError"
    elseif task.priority == "B" then pri_hl = "DiagnosticWarn"
    elseif task.priority == "C" then pri_hl = "DiagnosticInfo"
    elseif task.priority == "T" then pri_hl = "Special"
    end
    ...
    attach_mappings = function(prompt_bufnr, map)
      ...
      actions.select_default:replace(function()
        ...
        elseif selection then
          vim.notify("Selected task #" .. selection.value.n .. ": " .. selection.value.raw, vim.log.levels.INFO, { title = "Tuxedo" })
        end
      end)
      ...
      local do_complete = function()
        ...
        if selection then
          M.complete_task(selection.value.n, function(success)
            if success then
              vim.notify("Task #" .. selection.value.n .. " marked done!", vim.log.levels.INFO, { title = "Tuxedo" })
              actions.close(prompt_bufnr)
              -- Re-open refreshed picker
              M.project_todos()
            else
              vim.notify("Failed to complete task #" .. selection.value.n, vim.log.levels.ERROR, { title = "Tuxedo" })
            end
          end)
        end
      end
